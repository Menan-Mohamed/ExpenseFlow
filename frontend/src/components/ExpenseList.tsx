...
export function ExpenseList({ expenses, onEdit, onDelete, onSubmit, onDetails, pagination, onPageChange }: ExpenseListProps) {
  return (
    <section className="expense-list">
      ...
      {expenses.length === 0 ? <p className="empty-state">No expenses yet. Add your first purchase to get started.</p> : <div className="expense-items">{expenses.map((expense) => <article className="expense-item" key={expense.id}>
        <div className="expense-summary"><h3>{expense.title}</h3><p>{expense.category_name} · {expense.spent_date}</p>{expense.description && <p className="expense-description">{expense.description}</p>}</div>
        <div className="expense-meta"><strong>{Number(expense.amount).toFixed(2)}</strong><span className={`state state-${expense.state}`}>{expense.state}</span><div className="expense-actions"><button className="text-button" type="button" onClick={() => onDetails(expense)}>Details</button>{expense.state === 'draft' && <><button className="text-button" type="button" onClick={() => onEdit(expense)}>Edit</button><button className="text-button" type="button" onClick={() => onSubmit(expense)}>Submit</button><button className="danger-button" type="button" onClick={() => onDelete(expense)}>Delete</button></>}</div></div>
      </article>)}</div>}
