...
import { getCategories, type Category, type Expense, type ExpenseDetails, type ExpenseInput } from '../services/expenses'
import {
...
export function ManagerPage({ onLogout }: { onLogout: () => void }) {
  const [section, setSection] = useState<'expenses' | 'members' | 'reviews'>('expenses')
  const [expenses, setExpenses] = useState<Expense[]>([])
  const [categories, setCategories] = useState<Category[]>([])
  const [members, setMembers] = useState<ManagerMember[]>([])
  const [reviews, setReviews] = useState<ManagerReviewExpense[]>([])
  const [expensePage, setExpensePage] = useState({ page: 1, total_pages: 1, total_count: 0, per_page: 5 })
  ...
  const [reviewDetails, setReviewDetails] = useState<ManagerReviewDetails | null>(null)
  ...
  function loadMembers(page = 1) {
    getManagerMembers(page).then((result) => {
  ...
  function loadReviews(page = 1) {
    getManagerReviews(page).then((result) => {
  ...
  async function saveExpense(input: ExpenseInput) {
    setSaving(true)
    try {
      const saved = editing ? await updateManagerExpense(editing.id, input) : await createManagerExpense(input)
      setEditing(null)
      if (!editing) setFormKey((current) => current + 1)
      setExpenses((current) => editing ? current.map((item) => item.id === saved.id ? saved : item) : [saved, ...current])
    } catch (reason) {
      showError(reason instanceof Error ? reason : new Error('Unable to save expense.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function removeExpense(expense: Expense) {
    ...
    try {
      ...
      showError(reason instanceof Error ? reason : new Error('Unable to delete expense.'))
    }
  }
  ...
  async function submitExpense(expense: Expense) {
    try {
      ...
      showError(reason instanceof Error ? reason : new Error('Unable to submit expense.'))
    }
  }

  async function review(expense: ManagerReviewExpense, action: 'approve' | 'reject') {
    const comment = window.prompt(action === 'reject' ? 'Reason for rejection (required)' : 'Comment (optional)', '')
    if (action === 'reject' && !comment) return
    try {
      if (action === 'approve') await approveManagerExpense(expense.id, comment ?? '')
      else await rejectManagerExpense(expense.id, comment ?? '')
      loadReviews(reviewPage.page)
    } catch (reason) {
      showError(reason instanceof Error ? reason : new Error('Unable to update expense.'))
    }
  }
    ...
    <main className="dashboard">
      ...
      <section className="employee-content">
        ...
        <nav className="admin-tabs" aria-label="Manager sections">
          <button className={section === 'expenses' ? 'admin-tab active' : 'admin-tab'} onClick={() => setSection('expenses')}>My expenses</button>
          <button className={section === 'members' ? 'admin-tab active' : 'admin-tab'} onClick={() => setSection('members')}>Team members</button>
